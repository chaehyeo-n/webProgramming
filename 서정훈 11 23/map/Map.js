...
function addPlaceListResult(places) {
    ...
    itemStr += '    <aside style="float: right; width: 170px" class="unselected">';
    itemStr += '        <input style="zoom:2.0;" type="checkbox" id="fixed_time_checker" name="time_fix"';
    itemStr += '        <label for="fixed_time_checker">시간 고정하기</label>';
    itemStr += '        <div id="time_select"><h5 id="time_text">12:00~12:00</h5>';
    itemStr += '            <div id=time_button_group>'
    itemStr += '                시작<input type="time" class="time_button" id="start" value="12:00">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
    itemStr += '                종료<input type="time" class="time_button" id="end" value="12:00">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
    itemStr += '            </div>'
    itemStr += '        </div></aside>';
	...
	var p = [ places.place_name, places.x, places.y ];
	
	if (places.road_address_name) p.push(places.road_address_name); // 도로명 주소
	else p.push("");
	
	if (places.address_name) p.push(places.address_name); // 지번 주소
	else p.push("");
	
	if (places.phone) p.push(places.phone); // 전화 번호
	else p.push("");
	
	p_list.push(p);
	place_result_hashmap.set(day, p_list);
